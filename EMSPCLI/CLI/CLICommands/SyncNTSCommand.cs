...
namespace cloud.charging.open.EMSP.CommandLine
{
    ...
    /// <summary>
    /// Ask this EMSP's time servers what the time is, or one of them
    /// everything.
    /// </summary>
    /// <remarks>
    /// Without a server, the same thing the NTS page does with 'Sync now'. Both
    /// end in SyncTimeAsync, which asks the same group of servers, writes every
    /// step into the log and keeps the result as the last synchronisation the
    /// page shows - so the log book reads the same whichever of the two was
    /// used, apart from the one line that says who asked. A command that did
    /// any of that itself would be the beginning of two EMSPs disagreeing about
    /// what they did.
    ///
    /// With one, the same thing as that server's Test button on the page:
    /// TestTimeServerAsync, on the ports the server is configured with, step by
    /// step. Only a server of this EMSP is tested. Anything else is answered
    /// with the servers there are, and nothing is asked - a name that was
    /// mistyped would otherwise be a key exchange with whoever answers to it.
    ///
    /// Neither steps the clock: they say whether the time servers can be
    /// reached and what they think of the local clock.
    ///
    /// The charging station's command, as the vehicle's is: the three of them
    /// ask the same kind of group, and should be asked the same way.
    /// </remarks>
    /// <param name="CLI">The command line of the EMSP to ask.</param>
    public class SyncNTSCommand(ProviderCLI CLI) : ACLICommand<ProviderCLI>(CLI),
                                                   ICLICommand
    {
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length == 2)
            {
                ...
                var server  = cli.EMSP.TimeSources.Sources.FirstOrDefault(source => String.Equals(source.Hostname.Trimmed,
                                                                                                  wanted,
                ...
                if (server is null)
                    return [ $"'{Arguments[1]}' is none of this EMSP's time servers, which are {String.Join(", ", TimeServers())}." ];
                ...
                cli.EMSP.Log.Info(
                    $"Somebody at the command line asked this EMSP to test the time server '{host}'.",
                    "nts", "test", "cli"
                );

                return Tested(await cli.EMSP.TestTimeServerAsync(host, CancellationToken));
            ...
            // which the EMSP cannot tell apart. Said before anything is asked,
            // because the entries that follow record which servers were asked
            // and what came of it, and nothing in them says who wanted it.
            cli.EMSP.Log.Info(
                "Somebody at the command line asked this EMSP to synchronise its time.",
                "nts", "test", "cli"
            );

            var result = await cli.EMSP.SyncTimeAsync(CancellationToken);
        ...
        /// <summary>
        /// The time servers of this EMSP, switched on or not, as somebody types
        ...
        private IEnumerable<String> TimeServers()
            ...
            => cli.EMSP.TimeSources.Sources.Select(source => source.Hostname.Trimmed);
        ...
        /// got to is the whole of what somebody needs - the certificate chain
        /// among them. The log has only the beginning and the end of it, as it
        /// has for the button.
        ///
        ...
        /// and an EMSP in a German locale would otherwise write "+1,2 ms" in
        /// the middle of one.
        /// </summary>
